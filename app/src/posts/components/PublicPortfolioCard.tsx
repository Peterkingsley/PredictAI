...
export function PublicPortfolioCard({ portfolio, onOpen }: { portfolio: PublicPortfolioSnapshot; onOpen?: () => void }) {
  ...
  const content = <>
    <View style={styles.heading}><Text style={styles.title}>{portfolio.timeframe} Portfolio</Text>{portfolio.showPerformance ? <Text style={[styles.performance, { color: positive ? colors.positive : colors.danger }]}>{formatPercent(portfolio.performancePercent, 2)}</Text> : null}</View>
    {portfolio.showChart ? <View style={styles.chart}><PerformanceLineChart color={positive ? colors.positive : colors.danger} height={82} values={chartValues}/></View> : null}
    {portfolio.positions.length ? <><Text style={styles.holdingsTitle}>Holdings</Text><View style={styles.positions}>{portfolio.positions.slice(0, 5).map((position) => <View key={position.id} style={styles.row}><View style={styles.asset}><Text style={styles.symbol}>{position.assetSymbol}</Text><Text style={styles.assetName}>{position.assetName}</Text></View>{portfolio.showAllocation && position.allocationPercent !== undefined ? <Text style={styles.rowValue}>{position.allocationPercent}%</Text> : portfolio.showPerformance ? <Text style={[styles.rowPerformance, { color: position.performancePercent >= 0 ? colors.positive : colors.danger }]}>{formatPercent(position.performancePercent)}</Text> : <Text style={styles.muted}>Public</Text>}</View>)}</View></> : null}
...
function SummaryRow({ label, positive, value }: { label: string; positive: boolean; value: string }) { return <View style={styles.metaRow}><Text style={styles.muted}>{label}</Text><Text style={[styles.summaryValue, { color: positive ? colors.positive : colors.danger }]}>{value}</Text></View>; }
