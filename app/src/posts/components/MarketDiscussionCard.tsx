...
export function MarketDiscussionCard({ marketId, onOpen }: { marketId: string; onOpen: () => void }) {
  ...
  return <Pressable onPress={onOpen} style={({ pressed }) => [styles.card, pressed && styles.pressed]}><View style={styles.icon}><Ionicons color="#43C6D5" name="chatbubbles-outline" size={20}/></View><View style={styles.copy}><Text style={styles.title}>Community discussion</Text><Text style={styles.meta}>{count ? `${count} posts with public market views` : 'Start the conversation on this market'}</Text></View><Text style={styles.open}>View</Text><Ionicons color={colors.textMuted} name="chevron-forward" size={17}/></Pressable>;
}
const styles = StyleSheet.create({ card: { minHeight: 70, marginTop: 12, padding: 12, borderRadius: 12, borderWidth: 1, borderColor: colors.border, backgroundColor: '#17191B', flexDirection: 'row', alignItems: 'center', gap: 10 }, pressed: { opacity: .65 }, icon: { width: 38, height: 38, borderRadius: 10, backgroundColor: '#15272A', alignItems: 'center', justifyContent: 'center' }, copy: { flex: 1 }, title: { color: colors.text, fontSize: 11, fontWeight: '700' }, meta: { color: colors.textMuted, fontSize: 8, marginTop: 4 }, open: { color: '#43C6D5', fontSize: 9, fontWeight: '700' } });
