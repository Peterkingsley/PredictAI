    ...
    <View style={styles.root}>
      ...
      >
        {announcements.map((announcement) => (
          <View key={announcement.title} style={[styles.slide, { width: pageWidth }]}>
            <View style={[styles.banner, { backgroundColor: announcement.background }]}>
              <View style={styles.copy}>
                <Text
                  allowFontScaling={false}
                  style={[styles.eyebrow, { color: announcement.accent }]}
                >
      ...
      <View style={styles.dots}>
        {announcements.map((announcement, index) => (
          <View key={announcement.title} style={[styles.dot, index === page && styles.dotActive]} />
        ))}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  root: { height: HERO_HEIGHT, backgroundColor: "#151719" },
  ...
  title: { color: colors.text, fontSize: 17, lineHeight: 22, fontWeight: "700", marginTop: 5 },
  description: { color: "#B2B4B9", fontSize: 11, lineHeight: 16, marginTop: 5 },
  iconCircle: {
    width: 62,
    height: 62,
    borderRadius: 31,
  ...
  dot: { width: 4, height: 4, borderRadius: 2, backgroundColor: "#72757B" },
  dotActive: { width: 12, backgroundColor: colors.text },
});
