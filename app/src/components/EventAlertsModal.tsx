  ...
  return (
    <View style={styles.settingRow}>
      <View style={styles.settingCopy}>
        <Text allowFontScaling={false} style={styles.settingLabel}>
          {label}
        </Text>
        <Text allowFontScaling={false} style={styles.settingDescription}>
          {description}
        </Text>
      </View>
      <Switch
...
export function EventAlertsModal({
  marketTitle,
  onClose,
  visible,
}: {
        ...
        <Pressable onPress={() => undefined} style={styles.sheet}>
          ...
          <View style={styles.header}>
            <View style={styles.headerCopy}>
              ...
              <Text allowFontScaling={false} numberOfLines={2} style={styles.marketTitle}>
                {marketTitle}
              </Text>
            </View>
          ...
          <SettingRow
            description="Get notified when the probability changes significantly."
            enabled={movementAlert}
            label="Probability movement"
            onChange={setMovementAlert}
          />
          {movementAlert ? (
            <View style={styles.thresholds}>
              <Text allowFontScaling={false} style={styles.fieldLabel}>
                Notify after a change of
              </Text>
              <View style={styles.thresholdRow}>
                {thresholds.map((value) => (
                  <Pressable
                    ...
                    style={[styles.threshold, threshold === value && styles.thresholdSelected]}
                  >
                    <Text
                      allowFontScaling={false}
                      style={[
                        styles.thresholdText,
                        threshold === value && styles.thresholdTextSelected,
                      ]}
                    >
                      {value}%
                    </Text>
          ...
          <SettingRow
            description="Receive a reminder before predictions close."
            enabled={closingReminder}
            label="Event closing"
            onChange={setClosingReminder}
          />
          <SettingRow
            description="Know when the final outcome is confirmed."
            enabled={resolutionAlert}
            label="Event resolved"
            onChange={setResolutionAlert}
          />
          <SettingRow
            description="Notify when an outcome reaches your target."
            enabled={targetAlert}
            label="Target probability"
            onChange={setTargetAlert}
          />
          {targetAlert ? (
            <View style={styles.targetField}>
              ...
              <View style={styles.inputRow}>
                ...
                />
                <Text allowFontScaling={false} style={styles.percent}>
                  %
                </Text>
              </View>
            </View>
          ...
          <View style={styles.save}>
            <PrimaryButton label="Save alerts" onPress={onClose} />
          </View>
        </Pressable>
      </Pressable>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  thresholdSelected: { backgroundColor: "#25272C", borderWidth: 1, borderColor: colors.text },
