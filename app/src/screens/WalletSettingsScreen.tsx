        ...
        {options.map((option) => (
          ...
          >
            <Text
              ...
              style={[styles.choiceText, option === value && styles.choiceTextSelected]}
            >
              {option}
            </Text>
          </Pressable>
...
function NavigationRow({
  ...
  return (
    ...
    >
      ...
      <Text allowFontScaling={false} style={styles.navigationLabel}>
        {label}
      </Text>
      {detail ? (
        <Text allowFontScaling={false} style={styles.navigationDetail}>
          {detail}
        </Text>
      ) : null}
...
export function WalletSettingsScreen({
  ...
  onOpenDeposit: () => void;
  onOpenHistory: () => void;
  onUpdate: (settings: WalletSettings) => void;
  ...
  const addAddress = () => {
    ...
    if (!settings.trustedAddresses.includes(address))
      update({ trustedAddresses: [...settings.trustedAddresses, address] });
    setNewAddress("");
    setAddressError("");
  };
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Section icon="shield-checkmark-outline" title="Security">
          <ToggleRow
            description="Require biometric authentication before withdrawals."
            label="Biometric approval"
            onChange={(requireBiometrics) => update({ requireBiometrics })}
            value={settings.requireBiometrics}
          />
          <ToggleRow
            description="Review withdrawal details before funds are sent."
            label="Withdrawal confirmation"
            onChange={(withdrawalConfirmation) => update({ withdrawalConfirmation })}
            value={settings.withdrawalConfirmation}
          />
          <ChoiceRow
            label="Automatic wallet lock"
            onChange={(autoLock) => update({ autoLock })}
            options={["1 min", "5 min", "15 min", "Never"] as const}
            value={settings.autoLock}
          />
          <NavigationRow
            detail={`${settings.trustedAddresses.length}`}
            icon="bookmark-outline"
            label="Trusted addresses"
            onPress={() => setManagement("addresses")}
          />
        </Section>
        ...
        <Section icon="swap-horizontal-outline" title="Transaction preferences">
          <ChoiceRow
            ...
            options={["Ethereum", "Polygon", "Arbitrum", "Base"] as const}
          ...
          <View style={styles.fieldBlock}>
            ...
            <View style={styles.field}>
              <TextInput
                ...
                onChangeText={(withdrawalLimit) =>
                  update({ withdrawalLimit: withdrawalLimit.replace(/[^0-9.]/g, "") })
                }
                placeholder="0.00"
              ...
              />
              <Text allowFontScaling={false} style={styles.fieldSuffix}>
                USDC
              </Text>
            </View>
          </View>
          <ChoiceRow
            ...
            onChange={(feeSpeed) => update({ feeSpeed })}
            options={["Standard", "Fast"] as const}
            value={settings.feeSpeed}
          />
          <ToggleRow
            description="Notify when deposits and withdrawals change status."
            label="Transaction notifications"
            onChange={(transactionNotifications) => update({ transactionNotifications })}
        ...
        <Section icon="options-outline" title="Display preferences">
          <ChoiceRow
            ...
            options={["USD", "NGN", "USDC"] as const}
            value={settings.currency}
          />
          <ToggleRow
            description="Mask wallet balances whenever the wallet opens."
            label="Hide balances by default"
            onChange={(hideBalances) => update({ hideBalances })}
            value={settings.hideBalances}
          />
          <ChoiceRow
            label="Sort assets by"
            onChange={(assetSort) => update({ assetSort })}
            options={["Balance", "Name", "Performance"] as const}
            value={settings.assetSort}
          />
          <ToggleRow
            description="Use smaller rows to show more assets."
            label="Compact asset view"
            onChange={(compactView) => update({ compactView })}
            value={settings.compactView}
          />
          <ChoiceRow
            label="Wallet mode"
            onChange={(walletMode) => update({ walletMode })}
            options={["Prediction", "Trading"] as const}
            value={settings.walletMode}
          />
        </Section>

        <Section icon="wallet-outline" title="Wallet management">
          <ToggleRow
            description="Show USDC in deposits, withdrawals, and balances."
            label="USDC"
            onChange={(value) => {
              if (value || settings.supportedAssets.USDT)
                update({ supportedAssets: { ...settings.supportedAssets, USDC: value } });
            }}
            value={settings.supportedAssets.USDC}
          />
          <ToggleRow
            description="Show USDT in deposits, withdrawals, and balances."
            label="USDT"
            onChange={(value) => {
              if (value || settings.supportedAssets.USDC)
                update({ supportedAssets: { ...settings.supportedAssets, USDT: value } });
            }}
            value={settings.supportedAssets.USDT}
          />
          <NavigationRow icon="qr-code-outline" label="Deposit addresses" onPress={onOpenDeposit} />
          <NavigationRow
            detail={settings.connectedWallet ?? "None"}
            icon="link-outline"
            label="Connected wallets"
            onPress={() => setManagement("wallets")}
          />
          <NavigationRow
          ...
          >
            <Ionicons
              color={colors.accent}
              name={exported ? "checkmark-circle-outline" : "download-outline"}
              size={18}
            />
            <Text allowFontScaling={false} style={styles.exportText}>
              {exported ? "History export prepared" : "Export transaction history"}
            </Text>
          </Pressable>
        </Section>
      </ScrollView>
      ...
      <Modal
        animationType="slide"
        onRequestClose={() => setManagement(null)}
        transparent
        visible={management !== null}
      >
        <Pressable onPress={() => setManagement(null)} style={styles.overlay}>
          <Pressable onPress={() => undefined} style={styles.sheet}>
            ...
            {management === "addresses" ? (
              <>
                ...
                <Text allowFontScaling={false} style={styles.sheetDescription}>
                  Withdrawals to trusted addresses are easier to recognize during confirmation.
                </Text>
                ...
                {settings.trustedAddresses.length ? (
                  settings.trustedAddresses.map((address) => (
                    <View key={address} style={styles.addressRow}>
                      <Text allowFontScaling={false} numberOfLines={1} style={styles.address}>
                        {address}
                      </Text>
                      <Pressable
                        onPress={() =>
                          update({
                            trustedAddresses: settings.trustedAddresses.filter(
                              (item) => item !== address,
                            ),
                          })
                        }
              ...
              <>
                ...
                <Text allowFontScaling={false} style={styles.sheetDescription}>
                  Choose a wallet provider to connect for this prototype.
                </Text>
                {["MetaMask", "WalletConnect", "Coinbase Wallet"].map((wallet) => (
                  ...
                  >
                    <View style={styles.walletOptionIcon}>
                      <Ionicons color={colors.text} name="wallet-outline" size={20} />
                    </View>
                    <Text allowFontScaling={false} style={styles.walletOptionText}>
                      {wallet}
                    </Text>
                    {settings.connectedWallet === wallet ? (
                ...
                {settings.connectedWallet ? (
                  ...
                  >
                    <Text allowFontScaling={false} style={styles.disconnectText}>
                      Disconnect wallet
                    </Text>
                  </Pressable>
                ) : null}
            ...
            <View style={styles.done}>
              <PrimaryButton label="Done" onPress={() => setManagement(null)} />
            </View>
          </Pressable>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  choiceSelected: { backgroundColor: "#25272C", borderWidth: 1, borderColor: colors.text },
