...
export function EventSearchScreen({
  ...
  onPredict: (market: Market, outcome: Outcome) => void;
}) {
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        <Pressable accessibilityLabel="Back to home" hitSlop={10} onPress={onBack}>
        ...
        <View style={styles.resultHeading}>
          ...
          {query.trim() ? (
            <View style={styles.queryTag}>
              <Text allowFontScaling={false} numberOfLines={1} style={styles.queryText}>
                “{query.trim()}”
              </Text>
            </View>
          ) : null}
        ...
        {results.length > 0 ? (
          <View style={styles.results}>
            {results.map((market) => (
              <MarketCard
                ...
                onPredict={(outcome) => onPredict(market, outcome)}
                onPress={() => onMarket(market)}
              />
            ))}
          </View>
        ) : (
          <View style={styles.empty}>
            <View style={styles.emptyIcon}>
              <Ionicons color={colors.textMuted} name="search-outline" size={31} />
            </View>
            <Text allowFontScaling={false} style={styles.emptyTitle}>
              No events found
            </Text>
            <Text allowFontScaling={false} style={styles.emptyText}>
              Try another event name, participant, category, or outcome.
            </Text>
            <Pressable
              onPress={() => {
                setQuery("");
...
const styles = StyleSheet.create({
  ...
  filterActive: { backgroundColor: "#25272C", borderWidth: 1, borderColor: "#3A3D42" },
