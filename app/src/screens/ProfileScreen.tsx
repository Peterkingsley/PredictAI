...
const information: Record<
  Exclude<Panel, "edit" | "verification" | "password" | "sessions" | "signout" | "delete" | null>,
  { title: string; body: string }
> = {
  help: {
    title: "Help center",
    body: "Find answers about predictions, deposits, withdrawals, event resolution, and account access. For urgent account issues, use Contact support.",
  },
  support: {
    title: "Contact support",
    body: "Support requests will include your account ID and app diagnostics. A support messaging connection can be added when the backend support channel is configured.",
  },
  terms: {
    title: "Terms of use",
  ...
  privacy: {
    ...
    body: "PredictAI uses account, identity, device, and transaction information to provide and protect the service. Privacy controls and data requests are available from your account.",
  },
  rules: {
    title: "Prediction rules",
    body: "Each market resolves from the source listed in its event rules. Predictions cannot be changed after confirmation, and final settlement follows the published resolution result.",
  },
};
  ...
  return (
    ...
    >
      <View style={[styles.menuIcon, danger && styles.dangerIcon]}>
        <Ionicons color={danger ? colors.danger : colors.text} name={icon} size={18} />
      </View>
      <Text allowFontScaling={false} style={[styles.menuLabel, danger && styles.dangerText]}>
        {label}
      </Text>
      {detail ? (
        <Text allowFontScaling={false} style={styles.menuDetail}>
          {detail}
        </Text>
      ) : null}
        ...
        {options.map((option) => (
          ...
          >
            <Text
              ...
              style={[styles.choiceText, value === option && styles.choiceTextSelected]}
            >
              {option}
            </Text>
          </Pressable>
...
export function ProfileScreen({
  ...
  const info =
    panel && panel in information ? information[panel as keyof typeof information] : null;
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.profileCard}>
          <ProfilePhotoAvatar editable initials={initials} size={54} />
          <View style={styles.profileCopy}>
            ...
            <View style={[styles.status, verification === "In review" && styles.statusReview]}>
              <Text
                allowFontScaling={false}
                style={[styles.statusText, verification === "In review" && styles.statusReviewText]}
              >
                {verification}
              </Text>
            </View>
        ...
        >
          ...
          <View style={styles.verificationCopy}>
            <Text allowFontScaling={false} style={styles.verificationTitle}>
              Identity verification
            </Text>
            <Text allowFontScaling={false} style={styles.verificationText}>
        ...
        <Section title="Account security">
          ...
          />
          <ToggleRow
            description="Use your device biometrics when signing in."
            label="Biometric login"
            onChange={setBiometrics}
            value={biometrics}
          />
          <ToggleRow
            description="Require an additional code during sign-in."
        ...
        <Section title="Preferences">
          <ToggleRow
            description="Receive event, transaction, and security updates."
            label="Notifications"
          ...
          <MenuRow
            ...
            label="Language"
            onPress={() => setLanguage((current) => (current === "English" ? "French" : "English"))}
          />
          <Choices
          ...
          <View style={styles.limitBlock}>
            ...
            <Text allowFontScaling={false} style={styles.toggleDescription}>
              Maximum prediction amount per day.
            </Text>
            <View style={styles.limitField}>
              <TextInput
                keyboardType="decimal-pad"
                onChangeText={(value) => setResponsibleLimit(value.replace(/[^0-9.]/g, ""))}
                selectionColor={colors.accent}
                style={styles.limitInput}
                value={responsibleLimit}
              />
              <Text allowFontScaling={false} style={styles.limitCurrency}>
                USDC
              </Text>
            </View>
          </View>
        ...
        <Section title="Account actions">
          ...
          >
            <View>
              ...
              <Text allowFontScaling={false} style={styles.referralCode}>
                PRED-PETER
              </Text>
            </View>
            <View style={styles.copyCode}>
              <Ionicons
                color={colors.accent}
                name={referralCopied ? "checkmark" : "copy-outline"}
                size={17}
              />
          ...
          <Pressable onPress={() => undefined} style={styles.sheet}>
            ...
            {panel === "edit" ? (
              <>
                ...
                <TextInput onChangeText={setCountry} style={styles.field} value={country} />
                <View style={styles.sheetAction}>
                  <PrimaryButton label="Save profile" onPress={() => setPanel(null)} />
                </View>
              </>
            ) : null}
            {panel === "verification" ? (
              <>
                ...
                <Text allowFontScaling={false} style={styles.sheetText}>
                ...
                {verification === "Not verified" ? (
                  ...
                  <View style={styles.sheetAction}>
                    <PrimaryButton label="Done" onPress={() => setPanel(null)} />
                  </View>
                )}
              </>
            ) : null}
            {panel === "password" ? (
              <>
                ...
                />
                <View style={styles.sheetAction}>
                  <PrimaryButton label="Update password" onPress={() => setPanel(null)} />
                </View>
              </>
            ) : null}
            {panel === "sessions" ? (
              <>
                ...
                <View style={styles.session}>
                  <View style={styles.sessionIcon}>
                    <Ionicons color={colors.text} name="phone-portrait-outline" size={21} />
                  </View>
                  <View>
                    <Text allowFontScaling={false} style={styles.sessionTitle}>
                      Current Android device
                    </Text>
                    <Text allowFontScaling={false} style={styles.sessionDetail}>
            ...
            {panel === "signout" || panel === "delete" ? (
              <>
                <View style={[styles.confirmIcon, panel === "delete" && styles.deleteIcon]}>
                  <Ionicons
                    color={panel === "delete" ? colors.danger : colors.accent}
                    name={panel === "delete" ? "trash-outline" : "log-out-outline"}
                    size={30}
                  />
                </View>
                <Text allowFontScaling={false} style={styles.confirmTitle}>
                  {panel === "delete" ? "Delete account?" : "Sign out?"}
                </Text>
                <Text allowFontScaling={false} style={styles.confirmText}>
                  {panel === "delete"
                    ? "This prototype will sign you out. Permanent deletion must be completed by the account backend after identity confirmation."
                    : "You will need your credentials to sign in again."}
                </Text>
                <View style={styles.confirmActions}>
                  <Pressable onPress={() => setPanel(null)} style={styles.cancel}>
                  ...
                  <View style={styles.confirmButton}>
                    <PrimaryButton
                      label={panel === "delete" ? "Continue" : "Sign out"}
                      onPress={onSignOut}
                    />
                  </View>
                </View>
