...
export function AIAnalysisScreen({
  ...
  market: Market;
  onBack: () => void;
  onShareToPosts?: (analysis: AIAnalysis) => void;
}) {
  ...
  useEffect(() => {
    ...
    setFailed(false);
    setCompletedSteps(0);
    const timers = getAnalysisSteps(market.category).map((_, index) =>
      setTimeout(() => active && setCompletedSteps(index + 1), 260 * (index + 1)),
    );
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.marketTag}>
          <Text allowFontScaling={false} style={styles.marketTagText}>
            {market.category} · {market.subcategory}
          </Text>
        </View>
        <Text allowFontScaling={false} style={styles.marketTitle}>
          {market.title}
        </Text>

        {!analysis && !failed ? (
          <AIAnalysisLoading category={market.category} completedSteps={completedSteps} />
        ) : null}
        {failed ? (
          <View style={styles.error}>
            <View style={styles.errorIcon}>
              <Ionicons color={colors.danger} name="alert-circle-outline" size={31} />
            </View>
          ...
          <>
            <View style={styles.reportHeading}>
              <View>
                ...
                <Text allowFontScaling={false} style={styles.reportTitle}>
                  Intelligence report
                </Text>
              </View>
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeading}>
                ...
                <Text allowFontScaling={false} style={styles.signalScore}>
                  {analysis.sentimentScore}/100
                </Text>
              </View>
              {analysis.signals.map((signal) => (
                <AISignalRow key={signal.name} signal={signal} />
              ))}
            </View>
            <View style={styles.section}>
              <Text allowFontScaling={false} style={styles.sectionTitle}>
                Why PredictAI thinks this
              </Text>
            ...
            <View style={styles.section}>
              ...
              {analysis.risks.map((risk) => (
                <View key={risk} style={styles.risk}>
                  ...
                  <Text allowFontScaling={false} style={styles.riskText}>
                    {risk}
                  </Text>
                </View>
              ))}
            ...
            <View style={styles.updated}>
              ...
              <Text allowFontScaling={false} style={styles.updatedText}>
                Updated just now
              </Text>
              <Pressable
            ...
            {onShareToPosts ? (
              <Pressable onPress={() => onShareToPosts(analysis)} style={styles.share}>
                <Ionicons color={colors.buttonText} name="share-social-outline" size={17} />
                <Text style={styles.shareText}>Share analysis to Posts</Text>
              </Pressable>
            ) : null}
            <View style={styles.notice}>
              ...
              <Text allowFontScaling={false} style={styles.noticeText}>
                PredictAI provides model-generated estimates, not financial advice. Probabilities
                may be wrong and should not be your only basis for a prediction.
              </Text>
            </View>
          </>
        ) : null}
