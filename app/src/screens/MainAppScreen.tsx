...
export function MainAppScreen({ email, onSignOut }: { email: string; onSignOut: () => void }) {
  ...
  return <View style={styles.root}><View style={styles.body}>{tab === 'predict' && <PredictScreen email={email} onMarket={setSelectedMarket} onSignOut={onSignOut} />}{tab === 'explore' && <ExploreScreen onMarket={setSelectedMarket} />}{tab === 'assets' && <AssetsScreen onHistory={() => setHistoryOpen(true)} />}</View><AppBottomNav active={tab} onChange={setTab} /></View>;
}
