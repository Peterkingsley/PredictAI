...
export function CryptoMarketDetailScreen({
  market,
  onAskAI,
  onBack,
  onOpenPosts,
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.legend}>
          {market.outcomes.slice(0, 4).map((outcome, index) => (
            <View key={outcome.label} style={styles.legendRow}>
              <View
                style={[
                  styles.legendDot,
        ...
        <View style={styles.marketToolbar}>
          ...
          <View style={styles.ranges}>
            ...
            <Ionicons color={colors.textMuted} name="list-outline" size={21} />
          </View>
        </View>
        ...
        <View style={styles.outcomes}>
          {(expanded ? market.outcomes : market.outcomes.slice(0, 3)).map((outcome) => (
            <Pressable
              key={outcome.label}
              onPress={() => selectOutcome(outcome)}
              style={[
                styles.outcomeRow,
                selectedOutcome === outcome.label && styles.outcomeSelected,
              ]}
            >
              <View style={styles.coin}>
                <Text allowFontScaling={false} style={styles.coinText}>
                  ₿
                </Text>
              </View>
              <Text allowFontScaling={false} style={styles.outcomeLabel}>
                {outcome.label}
              </Text>
              <Text allowFontScaling={false} style={styles.odds}>
                {outcome.odds}
              </Text>
              <View style={styles.probability}>
                <Text allowFontScaling={false} style={styles.probabilityText}>
                  {outcome.probability}%
                </Text>
              </View>
            </Pressable>
        ...
        {!expanded ? (
          <Pressable onPress={() => setExpanded(true)} style={styles.more}>
            ...
            <Ionicons color={colors.textMuted} name="chevron-down" size={19} />
          </Pressable>
        ) : null}
