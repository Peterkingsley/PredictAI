...
export function PredictScreen({ email, onMarket, onSignOut }: { email: string; onMarket: (market: Market) => void; onSignOut: () => void }) {
  ...
  return <View style={styles.root}>
    ...
    <View style={styles.list}>{visible.map((market) => <MarketCard key={market.id} market={market} onPress={() => market.category === 'Sports' || market.category === 'Crypto' ? onMarket(market) : market.outcomes.length > 2 ? setAllMarket(market) : onMarket(market)} onPredict={(outcome) => predict(market, outcome)} />)}</View>
    </ScrollView>
